...
<body style="margin:0px; padding:0px; font-family:${settings?.fontFamily || 'Arial, sans-serif'};">
  <center>
    ...
    state?.sections?.forEach(section => {
      ...
      // Outer margins: applied as cell padding on the row cell so the section
      // background (on the inner table) stays inside the margin area
      const marginTop = section.settings.marginTop || 0;
      const marginRight = section.settings.marginRight || 0;
      const marginBottom = section.settings.marginBottom || 0;
      const marginLeft = section.settings.marginLeft || 0;
      const sectionMarginStyle = (marginTop > 0 || marginRight > 0 || marginBottom > 0 || marginLeft > 0)
      ...
      html += `
                <!-- Section Start -->
                <tr>
                  <td align="center" valign="top"${sectionMarginStyle}>
      ...
      section.columns.forEach(col => {
        ...
        const colBgColor = colSettings.backgroundColor ? ` background-color:${colSettings.backgroundColor};` : '';
        const colPadding = ` padding:${colSettings.paddingTop || 0}px ${colSettings.paddingRight || 0}px ${colSettings.paddingBottom || 0}px ${colSettings.paddingLeft || 0}px;`;
        const colHeight = colSettings.height ? ` height:${colSettings.height}px; min-height:${colSettings.height}px;` : '';

        html += `
                                  <!-- Column -->
                                  <td width="${colWidthPx}" valign="top" class="mobile-stack" style="width:${colWidthPx}px; font-family:${settings.fontFamily || 'Arial, sans-serif'};${colBgColor}${colPadding}${colHeight}">`;
